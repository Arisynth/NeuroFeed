...
3. 避免幻觉（hallucination），**注意理清新闻中的人物、组织、事件等关系，仔细核对数据，避免错误的描述。**
4. 直接输出中文摘要内容，不要添加额外的解释或说明。不要包含当前日期、来源信息、参考链接等元数据；不要使用“新闻简报”、“摘要：”、“总结：”等任何形式的词语作为摘要内容的开头；不要包含"简报结束"、"以上就是..."等作为结尾。
5. 请严格遵守以上要求，确保生成的摘要符合预期
